...
local player = {};
	...
	player.core = (player.self).PlayerScripts.Core;
	player.paintscript = require((player.core).Paint);

	player.setColor = function(c: Color3)
		player.paintscript:SetColor(c);
	end;

	player.setPixel = function(n: number, c: Color3)
		player.setColor(c);
		firesignal(player.grid[tostring(n)].MouseButton1Down);
		firesignal(player.grid[tostring(n)].MouseButton1Up);
	end;
...
local lib = {};
	...
	function lib:copy(plr: string, a: number, t: number, n: bool, s: string)
		...
		for i, v in ipairs(order) do
			player.setPixel(v, target.grid[tostring(v)].BackgroundColor3);
	...
	function lib:import(i: string, t: number, n: bool, s: string, port: string)
		...
		for i, v in ipairs(order) do
			...
			player.setPixel(v + 1, Color3.fromRGB(r,g,b));
