-- Script dumper. This also works on Valyse
...
progressText.Text = string.format("Script Dumper V%d!", Settings.Version)
progressText.Color = Color3.new(1, 1, 1)
...
progressText.Text = string.format("%d scripts...", totalScriptCount)
...
for i=1, Settings.Threads do
	spawn(function()
		while #needsDecompile > 0 do
			...
			local success, result = pcall(decompile, info.Script, false, 30)
			final[info.Index] = (success and result or "-- Failed to decompile script (" .. result or "???" .. ")")
			wait()
			doneScripts = doneScripts + 1
			progressBar.Size = Vector2.new(500 * doneScripts / totalScriptCount, progressBar.Size.Y)
			progressText.Text = string.format("%d / %d", doneScripts, totalScriptCount)
		end
...
local saveName = string.format("%s (%d) [%d].rbxlx", string.gsub(gameName, "  ", " "), game.PlaceId, os.time())
writefile(saveName, table.concat(final, "\n"))
progressText.Text = string.format("File %q (%ds; %dscr)", saveName, tick() - startTime, totalScriptCount)
wait(5)
credits.Remove()
progressText.Remove()
